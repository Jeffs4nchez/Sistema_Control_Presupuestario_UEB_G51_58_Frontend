...
function PagBtnLiq({ children, onClick, disabled, active }) {
  return (
    <motion.button
...
export default function Liquidaciones() {
  ...
  const puedeLiquidar = can.registrarLiquidacion(user)
  ...
  useEffect(() => {
    ...
    if (cert.items?.length === 1 && puedeLiquidar) {
    ...
    <div style={{ minHeight: '100%', background: 'var(--page-bg)', fontFamily: 'var(--font-primary)' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.90)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(26,58,92,0.08)', boxShadow: '0 2px 16px rgba(26,58,92,0.06)', padding: '20px 28px', marginBottom: '24px' }}>
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '20px', fontWeight: 800, color: TEXT, letterSpacing: '-0.02em' }}>Gestión de Liquidaciones</h1>
      ...
      <div style={{ padding: '0 28px 28px' }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {certs.slice((certsPage - 1) * CERTS_LIMIT, certsPage * CERTS_LIMIT).map((cert, ci) => {
        ...
        {/* Pagination liquidaciones */}
        {(() => {
          const totalPages = Math.max(1, Math.ceil(certs.length / CERTS_LIMIT))
          if (totalPages <= 1) return null
          return (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', flexWrap: 'wrap', marginTop: '8px' }}>
              <PagBtnLiq onClick={() => setCertsPage(1)} disabled={certsPage === 1}>«</PagBtnLiq>
              <PagBtnLiq onClick={() => setCertsPage(p => Math.max(1, p - 1))} disabled={certsPage === 1}>‹</PagBtnLiq>
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                .filter(n => n === 1 || n === totalPages || Math.abs(n - certsPage) <= 1)
                .reduce((acc, n, i, arr) => { if (i > 0 && n - arr[i - 1] > 1) acc.push('...'); acc.push(n); return acc }, [])
                .map((n, i) => n === '...'
                  ? <span key={`e${i}`} style={{ padding: '6px 4px', color: MUTED, fontSize: '13px' }}>…</span>
                  : <PagBtnLiq key={n} onClick={() => setCertsPage(n)} active={certsPage === n}>{n}</PagBtnLiq>
                )
              }
              <PagBtnLiq onClick={() => setCertsPage(p => Math.min(totalPages, p + 1))} disabled={certsPage === totalPages}>›</PagBtnLiq>
              <PagBtnLiq onClick={() => setCertsPage(totalPages)} disabled={certsPage === totalPages}>»</PagBtnLiq>
            </div>
          )
        })()}
      </div>
