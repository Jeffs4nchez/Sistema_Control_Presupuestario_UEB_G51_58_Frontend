...
function PagBtnCP({ children, onClick, disabled, active }) {
  return (
    <motion.button
...
export default function CedulaPresupuestaria() {
  ...
  const [sinDatosAnio,      setSinDatosAnio]      = useState(false)
  ...
  const filterClientSide = (search, programa, actividad, fuente, source) => {
    ...
    if (!data.length) {
      setPdItems([])
      setPdTotales({ total_items: 0, total_codificado: 0, total_certificado: 0, total_liquidado: 0, total_saldo: 0, items_sin_saldo: 0 })
      return
    }
    let items = data
    if (search.trim()) {
    ...
    try {
      ...
      if (json.success) {
        ...
        if (json.data.length === 0) {
          setSinDatosAnio(true)
          setTimeout(() => setSinDatosAnio(false), 3500)
        } else {
          setSinDatosAnio(false)
        }
      }
    } catch { /* silent */ }
        ...
        >
          ...
          {activeTab === 'datos' && (
            <div style={{ padding: P, display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              >
                {pdLoading ? (
                  ...
                  <motion.div
                    ...
                    style={{ margin: '24px', background: '#fefce8', border: '1px solid #fde047', borderRadius: '12px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '12px' }}
                  >
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
                      <path d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="#ca8a04" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  ...
                  <div style={{ width: '100%', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                    <table className="ueb-table" style={{ minWidth: '860px', width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {pdItems.slice((pdPage - 1) * PD_LIMIT, pdPage * PD_LIMIT).map((item, idx) => {
                          ...
                          >
                            <td style={{ padding: '9px 6px', color: MUTED, fontSize: '12px', textAlign: 'center', overflow: 'hidden', fontVariantNumeric: 'tabular-nums' }}>{(pdPage - 1) * PD_LIMIT + idx + 1}</td>
              ...
              {/* Pagination cédula */}
              {(() => {
                const totalPages = Math.max(1, Math.ceil(pdItems.length / PD_LIMIT))
                if (totalPages <= 1) return null
                return (
                  <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', flexWrap: 'wrap' }}>
                    <PagBtnCP onClick={() => setPdPage(1)} disabled={pdPage === 1}>«</PagBtnCP>
                    <PagBtnCP onClick={() => setPdPage(p => Math.max(1, p - 1))} disabled={pdPage === 1}>‹</PagBtnCP>
                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                      .filter(n => n === 1 || n === totalPages || Math.abs(n - pdPage) <= 1)
                      .reduce((acc, n, i, arr) => { if (i > 0 && n - arr[i - 1] > 1) acc.push('...'); acc.push(n); return acc }, [])
                      .map((n, i) => n === '...'
                        ? <span key={`e${i}`} style={{ padding: '6px 4px', color: MUTED, fontSize: '13px' }}>…</span>
                        : <PagBtnCP key={n} onClick={() => setPdPage(n)} active={pdPage === n}>{n}</PagBtnCP>
                      )
                    }
                    <PagBtnCP onClick={() => setPdPage(p => Math.min(totalPages, p + 1))} disabled={pdPage === totalPages}>›</PagBtnCP>
                    <PagBtnCP onClick={() => setPdPage(totalPages)} disabled={pdPage === totalPages}>»</PagBtnCP>
                  </div>
                )
              })()}
            </div>
