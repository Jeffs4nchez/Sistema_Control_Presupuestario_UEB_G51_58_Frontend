              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 20px' }}>
                    ...
                    {[
                      { label: 'Clase de Registro',   key: 'clase_registro',    max: 30 },
                      { label: 'Clase de Gasto',       key: 'clase_gasto',       max: 30 },
                      { label: 'Tipo Doc. Respaldo',   key: 'tipo_doc_respaldo', max: 50 },
                      { label: 'Clase Doc. Respaldo',  key: 'clase_doc_respaldo',max: 50 },
                      { label: 'Memorando N° *',        key: 'seccion_memorando', max: 30 },
                    ].map(({ label, key, max }) => (
                      <div key={key} style={FIELD}>
                        <label style={LABEL}>{label}</label>
                        <input style={INPUT} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })}
                          maxLength={max} onFocus={focusIn} onBlur={focusOut} />
                      </div>
                    ))}
                  </div>
