...
import { ChevronLeft, ChevronRight, Trash2, Edit2, Eye, Banknote, Printer, Search, RefreshCw, Clock, X, CheckCircle, XCircle, AlertCircle, RotateCcw } from 'lucide-react'
...
export default function ListaCertificaciones({ refresh }) {
  ...
  const { user } = useAuth()
  const isDirector    = can.aprobarCertificacion(user)
  ...
  const cargarCertificados = async (showSkeleton = false) => {
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
                        {(() => {
                          ...
                          const btnErrado   = b(<Trash2 size={13}/>,        RED,    'rgba(185,28,28,0.10)',   'rgba(185,28,28,0.28)',    'Marcar como Errada',     () => handleErrar(cert))

                          let botones
                          if (isReadOnly) {
                            botones = puedeAuditoria ? [btnVer, btnImprimir, btnHistorial] : [btnVer, btnImprimir]
                          } else if (cert.estado === 'REGISTRADO') {
                            if (puedeAuditoria)              botones = puedeErrar ? [btnVer, btnAprobar, btnRechazar, btnErrado] : [btnVer, btnAprobar, btnRechazar]
                            else if (isDirector)             botones = puedeErrar ? [btnVer, btnAprobar, btnRechazar, btnErrado] : [btnVer, btnAprobar, btnRechazar]
                            else if (esAnalista && esPropio) botones = puedeErrar ? [btnVer, btnEditar, btnErrado] : [btnVer, btnEditar]
                            else                             botones = [btnVer]
                          } else if (cert.estado === 'RECHAZADO') {
                            if (puedeAuditoria)              botones = puedeErrar ? [btnVer, btnHistorial, btnErrado] : [btnVer, btnHistorial]
                            else if (isDirector)             botones = [btnVer]
                            else if (esAnalista && esPropio) botones = [btnVer, btnEditar, btnReenviar]
                            else                             botones = [btnVer]
                          } else if (cert.estado === 'APROBADO') {
                            if (puedeAuditoria)  botones = puedeErrar ? [btnVer, btnImprimir, btnLiq, btnHistorial, btnEditar, btnErrado] : [btnVer, btnImprimir, btnLiq, btnHistorial, btnEditar]
                            else if (isDirector) botones = puedeErrar ? [btnVer, btnImprimir, btnLiq, btnEditar, btnErrado] : [btnVer, btnImprimir, btnLiq, btnEditar]
                            else                 botones = esPropio ? [btnVer, btnImprimir, btnLiq, btnEditar] : [btnVer, btnImprimir, btnLiq]
                          } else if (cert.estado === 'LIQUIDADO') {
                            if (puedeAuditoria)  botones = [btnVer, btnImprimir, btnLiq, btnHistorial, btnEditar]
                            else if (isDirector) botones = [btnVer, btnImprimir, btnLiq, btnEditar]
                            else                 botones = esPropio ? [btnVer, btnImprimir, btnLiq, btnEditar] : [btnVer, btnImprimir, btnLiq]
                          } else {
                            // ERRADO u otros
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 22px', background: 'linear-gradient(135deg, #1a0808, #a42e2e)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <Trash2 size={16} color="rgba(255,255,255,0.80)" />
                  <div>
              ...
              <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                  ...
                  >
                    {errarModal.loading ? 'Procesando...' : <><Trash2 size={13} /> Confirmar</>}
                  </motion.button>
                </div>
              </div>
            </motion.div>
