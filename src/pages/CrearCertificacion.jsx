...
export default function CrearCertificacion({ onCreated }) {
  ...
  const [certificado, setCertificado] = useState({
    ...
    seccion_memorando: "", fecha_elaboracion: new Date().toISOString().slice(0, 10),
    id_unidad_requiriente: "", id_cedula_presupuestaria: "",
  })
  ...
  const handleCrearCertificado = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        fecha_elaboracion: certificado.fecha_elaboracion,
      ...
      setTimeout(() => {
        setCreateStep(-1)
        onCreated()
        setCertificado({ descripcion: "", clase_registro: "", clase_gasto: "", tipo_doc_respaldo: "", clase_doc_respaldo: "", seccion_memorando: "", fecha_elaboracion: new Date().toISOString().slice(0, 10), id_unidad_requiriente: "", id_cedula_presupuestaria: "" })
        setItemsAgregados([])
        setStep(1)
      }, 1500)
    } catch (err) {
  ...
  const handleAgregarUnidad = async () => {
    if (!formUnidad.nombre_entidad?.trim())       { setErrorUnidad("Nombre de unidad es requerido"); return }
    if (!formUnidad.responsable_entidad?.trim())  { setErrorUnidad("Responsable es requerido"); return }
    if (!formUnidad.correo_institucional?.trim()) { setErrorUnidad("Correo es requerido"); return }
    if (!formUnidad.correo_institucional.toLowerCase().endsWith('@ueb.edu.ec')) { setErrorUnidad("El correo debe pertenecer al dominio @ueb.edu.ec."); return }
    setErrorUnidad("")
    try {
      const token = Cookies.get("auth_token")
      const res = await axios.post(`${API_BASE}/certificacion/unidades-requirientes`, formUnidad, {
        headers: { Authorization: `Bearer ${token}` },
      })
      const nuevaUnidad = res.data.data
      setEntidades([...entidades, nuevaUnidad])
      setCertificado({ ...certificado, id_unidad_requiriente: nuevaUnidad.id_unidad_requiriente })
      setShowModalUnidad(false)
      setFormUnidad({ nombre_entidad: "", responsable_entidad: "", correo_institucional: "" })
      setErrorUnidad("")
      setSuccess("Unidad agregada exitosamente")
      setTimeout(() => setSuccess(""), 3000)
    } catch (err) {
      setErrorUnidad(err.response?.data?.message || "Error al agregar unidad")
    }
  }
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'var(--font-primary)' }}>
            ...
            <div style={SECTION_S}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ background: 'rgba(46,108,164,0.04)', border: '1px solid rgba(46,108,164,0.12)', borderRadius: '10px', padding: '10px 12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <motion.button
                      ...
                      onClick={() => setShowModalUnidad(true)}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  {[
                    ['Clase de Registro *',   'clase_registro',    ['COM']],
                    ['Clase de Gasto *',      'clase_gasto',       ['OGA']],
                    ['Tipo Doc. Respaldo *',  'tipo_doc_respaldo', ['COMPROBANTES ADMINISTRATIVOS DE GASTOS']],
                    ['Clase Doc. Respaldo *', 'clase_doc_respaldo',['COMPROMISO NORMAL OTROS GASTOS']],
                  ].map(([lbl, field, opts]) => (
                    <div key={field}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  ...
                  <div>
                    <label style={LABEL_S}>Fecha de Elaboración *</label>
                    <input
                      type="date"
                      value={certificado.fecha_elaboracion}
                      onChange={e => setCertificado({ ...certificado, fecha_elaboracion: e.target.value })}
                      style={INPUT_S} onFocus={focusIn} onBlur={focusOut} required
                    />
                  </div>
                </div>
      ...
      {/* Modal: Agregar Unidad Requiriente */}
      <AnimatePresence>
        {showModalUnidad && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            onClick={e => { if (e.target === e.currentTarget) { setShowModalUnidad(false); setErrorUnidad("") } }}
          >
            <motion.div
              initial={{ scale: 0.88, opacity: 0, y: 24 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
            ...
            >
              <div style={{ background: 'linear-gradient(135deg, #0d1f35, #1a3a5c)', padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                  onClick={() => { setShowModalUnidad(false); setErrorUnidad("") }}
              ...
              <div style={{ padding: '22px' }}>
                <form onSubmit={e => { e.preventDefault(); handleAgregarUnidad() }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '18px' }}>
                    {[
                      ...
                      <div key={field}>
                        ...
                        <input
                          type={type} required
                          value={formUnidad[field]}
                          onChange={e => setFormUnidad({ ...formUnidad, [field]: e.target.value })}
                  ...
                  {errorUnidad && (
                    <div style={{ marginBottom: '12px', padding: '9px 12px', background: 'rgba(185,28,28,0.08)', border: '1px solid rgba(185,28,28,0.25)', borderRadius: '8px', fontSize: '12px', color: '#b91c1c', fontWeight: 500 }}>
                      {errorUnidad}
                    </div>
                  )}
                  <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button
                      type="button" onClick={() => { setShowModalUnidad(false); setErrorUnidad("") }}
