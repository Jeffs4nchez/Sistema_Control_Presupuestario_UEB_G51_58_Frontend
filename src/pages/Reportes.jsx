...
function TablaCertificaciones({ data }) {
  ...
  return (
    <table style={{ width:'100%', borderCollapse:'collapse', border:'1.5px solid #000' }}>
      <thead>
        <tr>
          {['N° Certificado','Fecha','Estado','Unidad Requiriente','Año','Memorando','Monto Total','Elaborado Por'].map((h,i)=>(
            <th key={i} style={{...TH, textAlign: i===6?'right':'center'}}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
      ...
      <tfoot>
        <tr style={{ background:BEIGE }}>
          <td colSpan={6} style={{...TDR, fontWeight:'bold', fontSize:'9px', borderTop:'1.5px solid #000'}}>TOTAL PRESUPUESTARIO</td>
...
function EjecTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background:'rgba(255,255,255,0.97)', border:'1px solid rgba(46,49,164,0.18)', borderRadius:'10px', padding:'10px 14px', boxShadow:'0 4px 20px rgba(0,0,0,0.13)', fontSize:'12px', fontFamily:'var(--font-primary)' }}>
      ...
      <div style={{ fontWeight:800, color: barColor(d.pct), fontSize:'14px' }}>{Number(d.pct).toFixed(2)}%</div>
    </div>
  )
}
...
export default function Reportes() {
  ...
  const [ejecAgrupacion, setEjecAgrupacion] = useState('programa')
  const [printEjec,   setPrintEjec]   = useState(false)
  const [sinDatosModal, setSinDatosModal] = useState(false)
  ...
  const handleEjecucion = async () => {
    ...
    try {
      ...
      const data = Object.values(map).map(d => ({
        ...d,
        pct: d.codificado > 0 ? Math.round((d.certificado_total / d.codificado) * 10000) / 100 : 0,
      })).sort((a, b) => b.pct - a.pct)
      if (data.length === 0) {
        setEjec({ data:[], loading:false, error:'' })
        setSinDatosModal(true)
        setTimeout(() => setSinDatosModal(false), 3500)
      } else {
        setEjec({ data, loading:false, error:'' })
        setEjecAgrupacion(f.agrupacion)
      }
    } catch(e) {
  ...
  const handlePrintEjec = () => {
    ...
    const agrupadoPor = { programa:'Programa', actividad:'Actividad', fuente:'Fuente', item:'Ítem' }[ejecAgrupacion] || ''
                    ...
                    <AnimatePresence>
                      ...
                      {sinDatosModal && (
                        <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:8 }}
                          style={{ marginTop:'20px', background:'#fefce8', border:'1px solid #fde047', borderRadius:'12px', padding:'20px 24px', display:'flex', alignItems:'center', gap:'12px' }}>
                          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#ca8a04" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink:0 }}><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                          <span style={{ fontSize:'13px', fontWeight:600, color:'#854d0e', lineHeight:1.5 }}>
                            No hay datos para el año <strong>{selectedCedula?.anio || 'seleccionado'}</strong>. Carga primero la cédula presupuestaria del año.
                          </span>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    <AnimatePresence>
                      {ejec.data.length > 0 && (
                        ...
                        >
                          ...
                          {(() => {
                            const muchos    = ejec.data.length > 8
                            const barW      = muchos ? 52 : 80
                            const chartW    = muchos ? Math.max(700, ejec.data.length * barW) : undefined
                            const chartH    = 380
                            const xLabel    = { programa:'Código de Programa', actividad:'Código de Actividad', fuente:'Código de Fuente', item:'Código de Ítem' }[ejecAgrupacion] || ''
                            const marginBot = muchos ? 90 : 64
                            const marginL   = 68
                            ...
                            const barraChart = (
                              <BarChart
                                ...
                                height={chartH}
                                data={ejec.data}
                                margin={{ top:24, right:24, left:marginL, bottom:marginBot }}
                              >
                                ...
                                <XAxis
                                  ...
                                  label={{
                                    ...
                                    offset: muchos ? -26 : -24,
                                    fontSize: 11, fontWeight:700, fill:'#1a3a5c',
                                ...
                                <ReferenceLine y={100} stroke="#a42e2e" strokeDasharray="4 3" strokeWidth={1.5}
                                  label={{ value:'100%', position:'insideTopRight', fontSize:9, fill:'#a42e2e' }}
                                />
                                <Bar dataKey="pct" radius={[4,4,0,0]} maxBarSize={muchos ? 40 : 60}
                                  label={{ position:'top', formatter:v=>`${Number(v).toFixed(2)}%`, fontSize:muchos ? 9 : 10, fontWeight:700, fill:'#374151' }}
                                >
                            ...
                            return muchos ? (
                              <div style={{ overflowX:'auto', borderRadius:'8px', paddingBottom:'4px' }}>
                                {barraChart}
                              </div>
                            ) : (
            ...
            return (
              <>
                ...
                <BarChart width={chartW} height={chartH} data={ejec.data}
                  margin={{ top:16, right:56, left:64, bottom: ejec.data.length > 8 ? 70 : 50 }}
                >
                  ...
                  <XAxis dataKey="name" tick={{ fontSize:8, fill:'#333', fontFamily:'monospace' }}
                    ...
                    interval={0}
                    label={{
                      value: { programa:'Código de Programa', actividad:'Código de Actividad', fuente:'Código de Fuente', item:'Código de Ítem' }[ejecAgrupacion] || '',
                      position: 'insideBottom',
                      offset: -8,
                      fontSize: 8,
                      fontWeight: 700,
                  ...
                  <Bar dataKey="pct" radius={[3,3,0,0]} maxBarSize={ejec.data.length > 12 ? 30 : 50}
                    label={{ position:'top', formatter:v=>`${Number(v).toFixed(2)}%`, fontSize:8, fontWeight:700, fill:'#222' }}
                  >
                    {ejec.data.map((d,i) => <Cell key={i} fill={barColor(d.pct)} />)}
                  </Bar>
                </BarChart>
